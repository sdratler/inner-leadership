...
import { runtimeIntakeOpening, type PrivateIntakeOpening } from "./opening.ts";
...
export class PreEnrollmentService {
  ...
  async exchange(token: string): Promise<{ childSlotIds: readonly string[]; expiresAt: string; consent: PublicConsent; opening: PrivateIntakeOpening }> {
    ...
    return { childSlotIds: row.childSlotIds, expiresAt: row.expiresAt.toISOString(), consent: runtimePublicConsent(), opening: runtimeIntakeOpening(row.tokenDigest, row.stableLeadId) };
  }

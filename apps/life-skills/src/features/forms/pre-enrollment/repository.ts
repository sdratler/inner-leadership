...
export class SqlPreEnrollmentRepository implements PreEnrollmentRepository {
  ...
  async consumeToken(tokenDigest:string,at:Date):Promise<boolean>{return (await this.query(`UPDATE ls_intake.pre_enrollment_invitations SET consumed_at=$3 WHERE workspace_id=$1 AND token_digest=$2 AND consumed_at IS NULL AND revoked_at IS NULL`,[this.workspaceId,tokenDigest,at])).length===1;}
}

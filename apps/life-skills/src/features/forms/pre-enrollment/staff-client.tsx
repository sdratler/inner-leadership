...
export function IntakeStaffClient({ locale = "he", respondentOrigin }: { locale?: StaffLocale; respondentOrigin: string }) {
  ...
  const [mode, setMode] = useState<"login" | "invite" | "reset" | "forgot">("login");
  const [pending, setPending] = useState(false);
  const [resetRequestAccepted, setResetRequestAccepted] = useState(false);
  ...
  async function requestPasswordReset(form: FormData) {
    ...
    setStatus("");
    try {
      await publicAuthAction("reset/request", { email }, locale);
      setResetRequestAccepted(true);
      setStatus(t.resetRequestSuccess);
    } catch {
      resetRequestLock.current = false;
      setStatus(t.resetRequestError);
    } finally {
      setPending(false);
    }
  }
  ...
  if (!session)
    if (mode === "forgot")
      return (
        <form className={styles.card} dir={t.direction} action={(form) => void requestPasswordReset(form)}>
          ...
          <button disabled={pending || resetRequestAccepted} type="submit">
            {pending ? t.requestingReset : t.requestReset}
          </button>
          <button disabled={pending} type="button" onClick={() => { setStatus(""); setMode("login"); }}>
            {t.backToSignIn}
          </button>
  ...
  if (!session)
    return (
      <form className={styles.card} dir={t.direction} action={(form) => void login(form)}>
        ...
        <button disabled={pending} type="button" onClick={() => { resetRequestLock.current = false; setResetRequestAccepted(false); setStatus(""); setMode("forgot"); }}>
          {t.forgotPassword}
        </button>
        <p role="alert">{status}</p>
      </form>

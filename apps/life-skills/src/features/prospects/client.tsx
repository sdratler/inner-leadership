...
export function ProspectsClient({locale,initialFilter="all",embedded=false,clientCases=[],caseState=null,onRetryCases,showProspects=true}:{locale:Locale;initialFilter?:Preset;embedded?:boolean;clientCases?:readonly ClientCase[];caseState?:State|null;onRetryCases?:()=>void;showProspects?:boolean}){
 ...
 const content=<>
  ...
  {showProspects&&<div className="lsw-tabs" role="tablist">{filters.map(([key,label])=><button key={key} type="button" role="tab" aria-selected={preset===key} onClick={()=>setPreset(key)}>{label}</button>)}</div>}
  ...
  <section className="lsw-stack" aria-live="polite">{casesShown.map(row=><a className="lsw-card" key={row.id} href={`/${locale}/app/cases/${encodeURIComponent(row.id)}`}><h2>{row.displayName}</h2><p>{row.kind==="minor"?(locale==="he"?"ילד/ה":"Child"):(locale==="he"?"מבוגר/ת":"Adult client")} · {row.state}</p></a>)}{showProspects&&state==="ready"&&visible.map(row=><ProspectCard key={row.leadId} row={row} locale={locale} action={action}/>)}{!casesShown.length&&caseState!=="loading"&&caseState!=="error"&&((showProspects&&state==="ready"&&!visible.length)||(!showProspects&&caseState==="ready"))&&<p className="lsw-empty">{showProspects?t.empty:locale==="he"?"אין תיקי לקוחות שמתאימים לחיפוש.":"No client cases match this search."}</p>}</section>
  {showProspects&&state==="ready"&&pages>1&&<nav className="lsw-actions" aria-label={t.page}><button className="lsw-button lsw-button--secondary" disabled={page<=1} onClick={()=>setPage(value=>value-1)}>{t.previous}</button><span>{t.page} {page} / {pages}</span><button className="lsw-button lsw-button--secondary" disabled={page>=pages} onClick={()=>setPage(value=>value+1)}>{t.nextPage}</button></nav>}
  {showProspects&&<><details ref={addRef} id="add-prospect" className="lsw-card lsu-inline-create"><summary>{t.add}</summary><AddProspect locale={locale} action={action}/></details><p className="lsw-save-result" role="status">{status}</p></>}
 </>;

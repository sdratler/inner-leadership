...
export function ProspectsClient({locale,initialFilter="all"}:{locale:Locale;initialFilter?:Preset}){
 ...
 return <main className="lsw-main" lang={locale} dir={locale==="he"?"rtl":"ltr"}>
  <header className="lsw-page-header"><div><p className="lsw-eyebrow">{locale==="he"?"CRM פרטי":"Private CRM"}</p><h1>{t.title}</h1><p>{t.lead}</p></div></header>
  ...
  <details className="lsw-card"><summary>{t.add}</summary><AddProspect locale={locale} action={action}/></details>

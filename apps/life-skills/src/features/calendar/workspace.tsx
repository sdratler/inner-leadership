...
import type { AppointmentView, Availability, SchedulePage } from './types.ts';
import { useCalendarMutation, useDialogGuard } from './form-support.tsx';
import { AttendanceForm, AvailabilityForm, BookingForm, NoticeForm, PractitionerActionForm, type CaseChoice, type SaveForm } from './forms.tsx';
...
export function CalendarWorkspace({locale,role,initialDate,initialView,initialCaseId}:{locale:Locale;role:'parent'|'practitioner';initialDate:string;initialView:'day'|'week'|'month';initialCaseId:string}){
 ...
 const range=dateRange(date,view),basePath=`/${locale}/${practitioner?'app/calendar':'family/schedule'}`;
 ...
 useDialogGuard('ls-cal-availability',dirty,mutation.locked,locale,clearDirty);
 const load=useCallback(async (reset=true,after:string|null=null)=>{
  ...
  try{
   ...
   if(!practitioner&&!selectedCase){selectedCase=currentCases.find(c=>c.kind==='minor')?.id??'';if(selectedCase){setCaseId(selectedCase);return;}}
   ...
   if(practitioner){const windows=await calendarRead<Availability[]>('availability?'+new URLSearchParams({from:range.from,to:range.to}));if(current===generation.current)setAvailable(windows);}
   if(selectedCase){const value=await calendarRead<{attendedChildSessions:number}>('attendance-count?'+new URLSearchParams({caseId:selectedCase}));if(current===generation.current)setCount(value.attendedChildSessions);}else setCount(null);
  }catch{if(current===generation.current){setError(true);if(reset){setItems([]);setCases([]);}}}
  finally{if(current===generation.current)setLoading(false);}
 },[caseId,range.from,range.to,practitioner]);
 ...
 </>:!selected.notice&&!selected.replacementId&&<details className="ls-cal-section" open><summary>{t.noticeTitle}</summary><NoticeForm locale={locale} appointment={selected} manual={false} locked={mutation.locked} onDirty={setDirty} save={save}/></details>}
 ...
 <Dialog id="ls-cal-availability" title={t.addWindow} locale={locale} busy={mutation.locked}><AvailabilityForm locale={locale} locked={mutation.locked} onDirty={setDirty} save={save}/>{mutation.feedback}</Dialog></>}
 </main>;
}

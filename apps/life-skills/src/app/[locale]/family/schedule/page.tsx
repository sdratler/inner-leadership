...
export default async function Page({params,searchParams}:Props){
 ...
 try{await calendarPageSession('parent');}catch{return <main lang={locale} dir={locale==='he'?'rtl':'ltr'} className="ls-cal"><h1>{text(locale).familyTitle}</h1><p role="status">{text(locale).unavailable}</p><a href={`/${locale}/`}>{text(locale).today}</a></main>;}
 ...
 return <CalendarWorkspace locale={locale} role="parent" initialDate={date} initialView={view} initialCaseId={caseId}/>;
}

...
export default async function Page({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<{role?:string;page?:string}>}){
 ...
 return <SyntheticWorkspacePreview key={`${locale}:${role}:${q.page??""}`} locale={locale} role={role} page={q.page??""}/>;
}

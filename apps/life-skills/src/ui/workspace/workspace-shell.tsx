...
import { activeItem, breadcrumbItems, navigationGroups, primaryNavigation, settingsItems, workspaceHref, type NavItem, type WorkspaceRole } from "./navigation-model.ts";
...
export type WorkspaceShellProps = { locale: Locale; role: WorkspaceRole; pathname: string; caseId?: string | null; languageHref: string; children: ReactNode; toHref?: (path: string) => string; notice?: ReactNode };
export function WorkspaceShell({ locale, role, pathname, caseId, languageHref, children, toHref, notice }: WorkspaceShellProps) {
  ...
  const crumbs = breadcrumbItems(locale, role, pathname);
  return <div className={`lsw lsu lsu--${role}`} lang={locale} dir={locale === "he" ? "rtl" : "ltr"}>
    ...
    <header className="lsu-header">
      ...
      <div className="lsu-header-controls">
        ...
        <details ref={account} className="lsu-account"><summary aria-label={t.account} className="lsu-icon-control"><svg aria-hidden="true" viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.7"><circle cx="12" cy="8" r="3.5"/><path d="M4.5 21v-2a7.5 7.5 0 0 1 15 0v2"/></svg></summary><nav className="lsu-account-panel" aria-label={t.account}><strong>{t.settings}</strong><a href={href(settings)}>{t.settings}</a>{settingsItems(role).slice(0, 2).map(x => <a key={x.key} href={href(x.path)}>{x[locale]}</a>)}</nav></details>
    ...
    <div className="lsu-layout">
      <aside className="lsu-sidebar"><nav aria-label={t.nav}><p className="lsu-nav-heading">{t[role]}</p>{primaryNavigation[role].map(link)}{groups}<a className="lsu-nav-link lsu-settings-link" href={href(settings)} aria-current={pathname === `/${locale}/${settings}` ? "page" : undefined}>{t.settings}</a></nav><p className="lsu-sidebar-note">{t.privacy}</p></aside>
      <div className="lsu-content">
        {role !== "practitioner" && <nav className="lsu-family-primary" aria-label={t.nav}>{primaryNavigation[role].map(link)}</nav>}
    ...
    <dialog ref={drawer} id={drawerId} className="lsu-drawer" aria-labelledby={titleId} onCancel={event => { event.preventDefault(); close(); }} onClick={event => { if (event.target === event.currentTarget) close(); }}>
      <div className="lsu-drawer-inner"><header><h2 id={titleId}>{t.nav}</h2><button type="button" className="lsu-icon-control" aria-label={t.close} onClick={close}>×</button></header><nav aria-label={t.nav}>{primaryNavigation[role].map(link)}{groups}<a className="lsu-nav-link" href={href(settings)}>{t.settings}</a></nav></div>
    </dialog>
  </div>;
}

  ...
  return <section className="lsr">
    ...
    {section === "content_calendar" && <Section title={h[section]}>
      ...
      <div className="lsr-content-calendar-scroll" role="region" aria-label={word(locale, "Monthly content calendar", "יומן תוכן חודשי")} tabIndex={0}><div className="lsr-content-calendar-grid">
        ...
        {calendarDates(month).map((day, index) => <div className="lsr-content-day" key={day ?? `blank-${index}`} aria-label={day ?? undefined}><strong>{day?.slice(-2) ?? ""}</strong>{day && <ul>{(days.get(day) ?? []).map(item => { const recordedAt = publicationDisplayTime(item); return <li key={item.id}><time dateTime={recordedAt ?? undefined}>{recordedAt ? new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit", timeZone: CONTENT_TIMEZONE }).format(new Date(recordedAt)) : "—"}</time> · {creativeFor(item.assetId, item.creativeRevision)?.title ?? item.assetId}<small>{channelName(item.channel, locale)} · {item.destinationLabel} · {publicationStatusText(item, snapshot.creatives, locale)}</small></li>; })}</ul>}</div>)}
      </div></div>

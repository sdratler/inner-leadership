...
const sections = ["overview", "content_calendar", "creatives", "needs_approval", "publishing", "ads"] as const;
const headings = {
  en: { overview: "Overview", content_calendar: "Content calendar", creatives: "Creatives", needs_approval: "Needs approval", publishing: "Publishing", ads: "Ads" },
  he: { overview: "סקירה", content_calendar: "יומן תוכן", creatives: "קריאייטיב", needs_approval: "דורש אישור", publishing: "פרסום", ads: "מודעות" },
};
...
export function MarketingDashboard({ locale, snapshot, initialSection, initialFilter }: { locale: Locale; snapshot: MarketingSnapshot; initialSection?: string | undefined; initialFilter?: string | undefined }) {
  ...
  const inventoryCards = inventory ? [
    ...
    ["queued", word(locale, "Queued", "בתור"), inventory.queued, "publishing"],
    ["published", word(locale, "Published", "פורסמו"), inventory.published, "publishing"],
  ] as const : [];
  ...
  return <section className="lsr">
    ...
    <nav className="lsr-tabs" aria-label={word(locale, "Marketing sections", "חלקי השיווק")}>{sections.map(key => <a key={key} aria-current={section === key ? "page" : undefined} href={`/${locale}/app/marketing?section=${key}`}>{h[key]}</a>)}</nav>
    {section === "overview" && <>

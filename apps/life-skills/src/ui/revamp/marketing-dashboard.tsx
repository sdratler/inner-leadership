  ...
  return <section className="lsr">
    ...
    {section === "content_calendar" && <Section title={h[section]}>
      ...
      <div className="lsr-content-calendar-scroll" role="region" aria-label={word(locale, "Monthly content calendar", "יומן תוכן חודשי")} tabIndex={0}><div className="lsr-content-calendar-grid">
        ...
        {calendarDates(month).map((day, index) => <div className="lsr-content-day" key={day ?? `blank-${index}`} aria-label={day ?? undefined}><strong>{day?.slice(-2) ?? ""}</strong>{day && <ul>{(days.get(day) ?? []).map(item => <li key={item.id}><time dateTime={item.scheduledFor ?? undefined}>{item.scheduledFor ? new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit", timeZone: CONTENT_TIMEZONE }).format(new Date(item.scheduledFor)) : "—"}</time> · {creativeFor(item.assetId, item.creativeRevision)?.title ?? item.assetId}<small>{channelName(item.channel, locale)} · {publicationStatusText(item, snapshot.creatives, locale)}</small></li>)}</ul>}</div>)}
      </div></div>
      {(view === "all" || view === "queued") && <><h3>{word(locale, "Ordered planned and ready records", "רשומות מתוכננות ומוכנות לפי סדר")}: {queue.length}</h3><p className="lsr-help">{word(locale, "Ready but unscheduled records are included; this count is not the provider queue count.", "רשומות מוכנות אך לא מתוזמנות כלולות; המספר אינו מספר הפריטים בתור הספק.")}</p>{queue.length ? <ol className="lsr-content-queue">{queue.map(item => { const asset = creativeFor(item.assetId, item.creativeRevision); const source = safeMarketingUrl(asset?.sourceUrl ?? asset?.imageUrl ?? null, ["drive.google.com", "docs.google.com", "github.com"]); return <li key={item.id}><strong>{asset?.title ?? item.assetId}</strong> · {channelName(item.channel, locale)} · {item.scheduledFor ? <time dateTime={item.scheduledFor}>{contentTime(item.scheduledFor, locale)}</time> : word(locale, "Unscheduled", "ללא מועד")}<p>{publicationStatusText(item, snapshot.creatives, locale)}{item.errorCode ? ` · ${item.errorCode}` : ""}</p>{source && <a href={source} target="_blank" rel="noopener noreferrer">{word(locale, "Open source asset", "פתיחת קובץ המקור")}</a>}</li>; })}</ol> : <p>{word(locale, "No planned records in the registry; this is not a provider queue readback.", "אין רשומות מתוכננות במאגר; זו אינה קריאה ישירה של תור הספק.")}</p>}</>}
      {view !== "all" && view !== "queued" && <><h3>{word(locale, "Selected records", "רשומות שנבחרו")}: {calendarItems.length}</h3><ol className="lsr-content-queue">{calendarItems.slice().sort((a, b) => (a.scheduledFor ?? "9999").localeCompare(b.scheduledFor ?? "9999")).map(item => <li key={item.id}><strong>{creativeFor(item.assetId, item.creativeRevision)?.title ?? item.assetId}</strong> · {channelName(item.channel, locale)} · {item.scheduledFor ? <time dateTime={item.scheduledFor}>{contentTime(item.scheduledFor, locale)}</time> : word(locale, "No recorded date", "אין תאריך רשום")}<p>{publicationStatusText(item, snapshot.creatives, locale)}</p></li>)}</ol></>}

...
import { adjacentMonth, calendarDates, contentMonth, contentView, contentViewPublications, monthPublications, nextHebrewStatus, orderedPublicationQueue, publicationDisplayTime, publicationTimeIssue, publicationStatusText, CONTENT_TIMEZONE, type ContentView } from "../../features/marketing-overview/calendar-model.ts";
...
function timeIssueText(item: Publication, locale: Locale): string | null {
  const issue = publicationTimeIssue(item);
  return issue === "manual" ? word(locale, "Invalid manual report date — check source", "תאריך דיווח ידני שגוי — יש לבדוק במקור") : issue === "scheduled" ? word(locale, "Invalid scheduled date — check source", "תאריך מתוזמן שגוי — יש לבדוק במקור") : null;
}
...
export function MarketingDashboard({ locale, snapshot, initialSection, initialFilter, initialMonth, renderedAt }: { locale: Locale; snapshot: MarketingSnapshot; initialSection?: string | undefined; initialFilter?: string | undefined; initialMonth?: string | undefined; renderedAt: string }) {
  ...
  const creativeFor = (item: Publication) => snapshot.creatives.find(asset => asset.assetId === item.assetId && asset.revision === item.creativeRevision && asset.contentDigest === item.creativeDigest);
  ...
  return <section className="lsr">
    ...
    {section === "content_calendar" && <Section title={h[section]}>
      ...
      <div className="lsr-status"><strong>{word(locale, "Next planned Hebrew WhatsApp Status in the registry", "הסטטוס המתוכנן הבא בעברית במאגר")}</strong><p>{nextStatus?.scheduledFor ? <><time dateTime={nextStatus.scheduledFor}>{contentTime(nextStatus.scheduledFor, locale)}</time> · {creativeFor(nextStatus)?.title ?? nextStatus.assetId} · {publicationStatusText(nextStatus, snapshot.creatives, locale)}</> : word(locale, "No upcoming planned Hebrew Status appears in these records. The external publisher queue may differ.", "לא מופיע כאן סטטוס מתוכנן עתידי בעברית. ייתכן שהתור אצל ספק הפרסום שונה.")}</p></div>
      ...
      <div className="lsr-content-calendar-scroll" role="region" aria-label={word(locale, "Monthly content calendar", "יומן תוכן חודשי")} tabIndex={0}><div className="lsr-content-calendar-grid">
        ...
        {calendarDates(month).map((day, index) => <div className="lsr-content-day" key={day ?? `blank-${index}`} aria-label={day ?? undefined}><strong>{day?.slice(-2) ?? ""}</strong>{day && <ul>{(days.get(day) ?? []).map(item => { const recordedAt = publicationDisplayTime(item); return <li key={item.id}><time dateTime={recordedAt ?? undefined}>{recordedAt ? new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit", timeZone: CONTENT_TIMEZONE }).format(new Date(recordedAt)) : "—"}</time> · {creativeFor(item)?.title ?? item.assetId}<small>{channelName(item.channel, locale)} · {item.destinationLabel} · {publicationStatusText(item, snapshot.creatives, locale)}</small></li>; })}</ul>}</div>)}
      </div></div>
      {(view === "all" || view === "queued") && <><h3>{word(locale, "Ordered planned and ready records", "רשומות מתוכננות ומוכנות לפי סדר")}: {queue.length}</h3><p className="lsr-help">{word(locale, "Ready but unscheduled records are included; this count is not the provider queue count.", "רשומות מוכנות אך לא מתוזמנות כלולות; המספר אינו מספר הפריטים בתור הספק.")}</p>{queue.length ? <ol className="lsr-content-queue">{queue.map(item => { const asset = creativeFor(item); const source = safeMarketingUrl(asset?.sourceUrl ?? asset?.imageUrl ?? null, ["drive.google.com", "docs.google.com", "github.com"]); const recordedAt = publicationDisplayTime(item), issue = timeIssueText(item, locale); return <li key={item.id}><strong>{asset?.title ?? item.assetId}</strong> · {channelName(item.channel, locale)} · {item.destinationLabel} · {recordedAt ? <time dateTime={recordedAt}>{contentTime(recordedAt, locale)}</time> : issue ? <span role="alert">{issue}</span> : word(locale, "Unscheduled", "ללא מועד")}<p>{publicationStatusText(item, snapshot.creatives, locale)}</p><PublicationEvidence item={item} locale={locale}/>{source && <a href={source} target="_blank" rel="noopener noreferrer">{word(locale, "Open source asset", "פתיחת קובץ המקור")}</a>}</li>; })}</ol> : <p>{word(locale, "No planned records in the registry; this is not a provider queue readback.", "אין רשומות מתוכננות במאגר; זו אינה קריאה ישירה של תור הספק.")}</p>}</>}
      {(view === "all" || view === "drafts" || view === "published" || view === "history") && <><h3>{view === "all" ? word(locale, "Other registry records", "רשומות נוספות במאגר") : word(locale, "Selected records", "רשומות שנבחרו")}: {view === "all" ? nonQueuedItems.length : calendarItems.length}</h3><ol className="lsr-content-queue">{(view === "all" ? nonQueuedItems : calendarItems).slice().sort((a, b) => (publicationDisplayTime(a) ? Date.parse(publicationDisplayTime(a)!) : Number.POSITIVE_INFINITY) - (publicationDisplayTime(b) ? Date.parse(publicationDisplayTime(b)!) : Number.POSITIVE_INFINITY) || a.id.localeCompare(b.id)).map(item => { const recordedAt = publicationDisplayTime(item), issue = timeIssueText(item, locale); return <li key={item.id}><strong>{creativeFor(item)?.title ?? item.assetId}</strong> · {channelName(item.channel, locale)} · {item.destinationLabel} · {recordedAt ? <time dateTime={recordedAt}>{contentTime(recordedAt, locale)}</time> : issue ? <span role="alert">{issue}</span> : word(locale, "No recorded date", "אין תאריך רשום")}<p>{publicationStatusText(item, snapshot.creatives, locale)}</p><PublicationEvidence item={item} locale={locale}/></li>; })}</ol></>}

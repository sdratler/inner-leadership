...
describe('LS-030 real PostgreSQL contracts',()=>{
 ...
 test('marked demo appointments never reserve the practitioner real booking capacity',async()=>{
  ...
  await g.pool.query('INSERT INTO ls_demo.batches(workspace_id,batch_id,created_by) VALUES($1,$2,$3)',[g.workspaceId,batch,g.practitioner.actor.id]);
  await g.pool.query('INSERT INTO ls_demo.cases(workspace_id,case_id,batch_id,source_key) VALUES($1,$2,$3,$4)',[g.workspaceId,g.first.id,batch,'synthetic-calendar-case']);
  ...
  await expect(g.pool.query('INSERT INTO ls_demo.cases(workspace_id,case_id,batch_id,source_key) VALUES($1,$2,$3,$4)',[g.workspaceId,g.second.id,batch,'cannot-relabel-booked-real-case'])).rejects.toMatchObject({code:'23514'});

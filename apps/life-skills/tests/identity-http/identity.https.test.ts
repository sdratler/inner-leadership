...
before(async()=>{
 ...
 const files=await Promise.all(['0001_ls_foundation.sql','0010_ls_identity_cases_20260906.sql','0030_ls_calendar_attendance_20260907.sql','0095_ls_optional_child_accounts.sql','0097_ls_demo_provenance.sql','0098_ls_demo_calendar_isolation.sql'].map(async name=>{const sql=await readFile(resolve('migrations',name),'utf8');return {name,sql,checksum:createHash('sha256').update(sql).digest('hex')};}));

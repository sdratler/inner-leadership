...
const origin='https://127.0.0.1:3445',port=3103;
...
let app:ChildProcess|null=null,tls:ReturnType<typeof httpsServer>|null=null,f:Awaited<ReturnType<typeof fixture>>|null=null;
...
try{
 ...
 writeFileSync(auth,JSON.stringify({cases,caseId:f.first.id,foreignId,parent:f.parent.token,practitioner:f.practitioner.token}),{mode:0o600});
 ...
 });tls.listen(3445,'127.0.0.1');await once(tls,'listening');
 const tests=spawn(process.execPath,[resolve('node_modules/@playwright/test/cli.js'),'test','--config','tests/e2e/calendar/playwright.config.ts'],{env,stdio:['ignore','inherit','inherit']});
 const [code]=await once(tests,'exit');if(code!==0)throw new Error('CALENDAR_BROWSER_ACCEPTANCE_FAILED');
...
finally{if(tls)await new Promise<void>(r=>tls!.close(()=>r()));await terminate(app);await f?.pool.end();rmSync(folder,{recursive:true,force:true});}

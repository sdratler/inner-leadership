...
describe("accepted-request-only Gmail reset delivery",()=>{
 ...
 test.each(["en","he"])("%s reset points to existing locale client with fragment-only token",async locale=>{
  ...
  expect(sent.text).toContain(locale === "he" ? "15 דקות" : "15 minutes");expect(sent.text).toMatch(locale === "he" ? /15 תווים/ : /15 characters/);expect(sent.text).not.toMatch(locale === "he" ? /6 תווים/ : /6 characters/);
 });

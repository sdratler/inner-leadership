...
describe("owner intake preview", () => {
  ...
  it("retains Basic protection elsewhere and closes an absent config", () => {
    applyPreviewEnvironment();
    expect(proxy(new NextRequest("https://preview.example.test/he/preview")).status).toBe(401);
    expect(proxy(new NextRequest("https://preview.example.test/api/payments")).status).toBe(401);

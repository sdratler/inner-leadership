...
const data = { operationId: command.operationId, reply: "What is one small step that your child could choose?", copyAllowed: true, reviewFlags: [], suggestedRule: "", ruleScope: "", originalUrl: null,
...
describe("authenticated app to existing Scout bridge", () => {
  ...
  it("refuses a reply generated for a different operation", async () => {
    const fetcher = vi.fn().mockResolvedValue(Response.json({ ok: true, data: { ...data, operationId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" } }));
    await expect(requestCommunityReply(command, fetcher as typeof fetch,
      { LS_COMMUNITY_SCOUT_BRIDGE_SECRET: secret })).rejects.toMatchObject({ code: "UNAVAILABLE" });
  });
});

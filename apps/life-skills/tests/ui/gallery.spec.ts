...
for(const locale of ['he','en'] as const)for(const role of ['parent','practitioner'] as const)for(const width of [390,768,1440]){
 test(`${locale} ${role} ${width}: hydrated synthetic UI`,async({page})=>{
  ...
  if(width<=1023){const drawer=page.locator('.lsw-mobile-drawer>summary');await drawer.click();await expect(page.locator('.lsw-mobile-drawer__panel')).toBeVisible();if(width===390)await page.screenshot({path:`test-results/app-nav-${locale}-${role}-mobile-drawer.png`});await drawer.click();await drawer.focus();await page.keyboard.press('Enter');await expect(page.locator('.lsw-mobile-drawer__panel')).toBeVisible();await page.keyboard.press('Enter');}

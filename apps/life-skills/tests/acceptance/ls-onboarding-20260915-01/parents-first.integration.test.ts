...
let db:any,store:SqlReceiptStore,database:IdentityStore,dir:string;
let sequence=0;
const sessions:string[]=[];
function bind(){
  database={transaction:work=>db.transaction((tx:any)=>work({query:async(sql,params)=>{sessions.push(sql); return (await tx.query(sql,params)).rows;}}))};
...
function body(orderId?:string,changes:any={}){
  const id='SYNTH-'+(++sequence);
  return {eventId:id,providerAccountId:account,status:'succeeded',purpose:'first_session',orderId,transactions:[{transactionId:id,status:'succeeded',amountMinor:55000,currency:'ILS'}],...changes};
}
function receive(payload:any,alternateStore=store){
...
async function resultState(payload:any){const result=await receive(payload); assert.equal(result.status,200); return (result as any).result.state;}
...
function eligibility(orderId:string,changes:any={},currentStore=store){return manualSchedulingEligibility({orderId,consent,method:'card',...changes},currentStore,policy);}
...
test('migration checksum is exact; private tables deny public access',async()=>{
  ...
  assert.equal(createHash('sha256').update(bytes).digest('hex'),manifest.find((x:any)=>x.name==='0090_ls_parents_first.sql').sha256);
  await db.exec('CREATE ROLE synthetic_parent');
  assert.equal((await db.query("SELECT has_schema_privilege('synthetic_parent','ls_onboarding','USAGE') AS allowed")).rows[0].allowed,false);
  await assert.rejects(()=>db.transaction(async(tx:any)=>{await tx.exec('SET LOCAL ROLE synthetic_parent');await tx.query('SELECT * FROM ls_onboarding.provider_receipts');}),/permission denied/);
});
...
test('duplicates/concurrent events and different event IDs cannot double allocate',async()=>{
  ...
  assert.equal(events.filter((x:any)=>x.result?.state==='paid').length,1);
  assert.equal((await db.query('SELECT count(*)::int AS n FROM ls_onboarding.payment_allocations WHERE order_id=$1',[o.orderId])).rows[0].n,1);
...
test('failure, pending and out-of-order refund never grant a payment',async()=>{
  const o=await order();
  for(const state of ['failed','pending']) assert.equal(await resultState(body(o.orderId,{status:state})),state);
...
test('wrong amount/currency/account and mismatched sibling/order are held',async()=>{
  ...
  for(const changes of [{amountMinor:54999},{currency:'USD'},{status:'failed'}]){
    const b=body(o.orderId);b.transactions[0]={...b.transactions[0],...changes};assert.equal(await resultState(b),'unmatched');
  }
...
test('generic link stays unmatched; explicit authorized staff allocation is audited and single-use',async()=>{
  ...
  assert.equal((await db.query('SELECT count(*)::int AS n FROM ls_onboarding.staff_allocation_audits WHERE audit_record_id=$1',[input.auditRecordId])).rows[0].n,1);
...
test('provider absence/outage and persistence errors return retryable failure, then safe replay',async()=>{
  ...
  assert.equal((await db.query('SELECT count(*)::int AS n FROM ls_onboarding.provider_receipts WHERE event_key=$1',[account+':'+b.eventId])).rows[0].n,0);
  assert.equal((await db.query('SELECT count(*)::int AS n FROM ls_onboarding.payment_allocations WHERE order_id=$1',[o.orderId])).rows[0].n,0);
  assert.equal(await resultState(b),'paid');
